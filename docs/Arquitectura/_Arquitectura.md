...
| [[ADR-046 Alcance de la idempotencia\|046]] | Qué identifica una reserva de idempotencia, y qué operaciones adoptan el helper | Identidad completa **`(usuario_id, operacion, clave_idempotencia)`**, nunca solo la clave · `exigirNueva` compara el hash del cuerpo: mismo hash finalizado → replay (`200`), mismo hash sin finalizar → `409` en proceso, otro hash → `409` conflicto · vencida o con una respuesta transitoria (`≥500`) se reutiliza la fila · solo adoptan el helper las operaciones con efecto financiero o irreversible | — |
| [[ADR-049 Doble aprobación de retiros\|049]] | Cómo se exige la doble aprobación de un retiro grande, y por qué no alcanza con el CHECK de la base | `EstadoDeRetiro` (enum puro) decide `AUTORIZADA` vs `EN_REVISION` según umbral · `ck_retiro_doble_aprobacion` y la aplicación exigen la segregación solicitante≠aprobador **por separado**, deliberadamente · el `UPDATE` condicionado por estado reemplaza al lock explícito en la carrera entre aprobadores | — |
| [[ADR-050 Rate limiting distribuido en el borde\|050]] | Cómo se corta por tasa el login, el registro y las operaciones de dinero en el gateway, con varias réplicas | **Redis compartido** (`RedisRateLimiter`), no memoria local ni `limit_req` de NGINX · `KeyResolver` propio IP+`Authorization` crudo (el gateway no decodifica JWT) · filtro propio para `Retry-After` (la librería no lo pone) y para fallar CERRADO cuando Redis no responde (el default de la librería falla abierto) | — |
