 ...
 * pasos. Este helper la maneja de punta a punta — formulario real, dos llamadas a
 ...
 *
 * El HTML de desarrollo no trae la meta del gateway: esta prueba la inyecta en la
 * respuesta de `/ingreso`. La ruta `/api/v1` pasa a Prism por el proxy E2E del
 * servidor de desarrollo, sin cambiar el HTML de producción ni el caso E2E de
 * configuración ausente.
 *
 * Lo único sintético en la respuesta de autenticación es el segundo `tokenAcceso`. `identidad` no tiene backend real
...
export async function iniciarSesionDePrueba(page: Page, permisos: readonly string[], rol = 'oficial'): Promise<void> {
  await page.route('**/ingreso', async (route) => {
    ...
    await route.fulfill({
      response: respuesta,
      body: html.replace('</head>', '<meta name="aportaya-gateway" content="/api/v1"></head>'),
    })
  }, { times: 1 })
