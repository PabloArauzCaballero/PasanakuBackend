...
class ResultadoValidacionGateway {
  const ResultadoValidacionGateway.valida(this.url) : valida = true, motivo = null;

  const ResultadoValidacionGateway.invalida(this.motivo) : valida = false, url = null;
  ...
  @override
  String toString() =>
      valida ? 'ResultadoValidacionGateway.valida($url)' : 'ResultadoValidacionGateway.invalida($motivo)';
}
...
ResultadoValidacionGateway validarGateway(
  ...
  if (cruda.startsWith('//')) {
    return const ResultadoValidacionGateway.invalida('esquema-relativo-a-protocolo');
  }
