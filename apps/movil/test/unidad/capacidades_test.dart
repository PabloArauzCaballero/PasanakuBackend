...
void main() {
  group('capacidadesDeLaPlataforma -- catorce casos (7 puertos x 2 plataformas)', () {
    ...
    test('iOS: AlmacenSeguro soportado (Keychain vía flutter_secure_storage)', () {
      expect(ios.almacenSeguro, GradoDeSoporte.soportado);
    });

    test('iOS: Conectividad soportada (connectivity_plus es multiplataforma real)', () {
    ...
    test('iOS: AvisosPush NO soportado (el canal de Android habla FCM, no APNs)', () {
    ...
    test('iOS: ProteccionPantalla NO soportada (FLAG_SECURE es de Android)', () {
  ...
  group('seguridadCriticaSoportada', () {
    test('Android: soportada (almacén y protección de pantalla plenos)', () {
      expect(
        capacidadesDeLaPlataforma(forzarIOSParaPruebas: false).seguridadCriticaSoportada,
        isTrue,
      );
    });
    ...
    test('iOS: NO soportada hoy (protección de pantalla no soportada)', () {
      expect(
        capacidadesDeLaPlataforma(forzarIOSParaPruebas: true).seguridadCriticaSoportada,
        isFalse,
      );
    });
  });
  ...
  group('los adaptadores "no soportado en iOS" nunca abren un canal nativo', () {
    setUp(() {
      ...
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        ...
        (call) => fail('BiometriaIos invocó un canal nativo: ${call.method}'),
      );
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(
        const MethodChannel('bo.aportaya/avisos_push'),
        (call) => fail('AvisosPushIos invocó un canal nativo: ${call.method}'),
      );
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(
        const MethodChannel('bo.aportaya/proteccion_pantalla'),
        (call) => fail('ProteccionPantallaIos invocó un canal nativo: ${call.method}'),
      );
    });

    tearDown(() {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(const MethodChannel('bo.aportaya/biometria'), null);
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(const MethodChannel('bo.aportaya/avisos_push'), null);
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(
              const MethodChannel('bo.aportaya/proteccion_pantalla'), null);
    });

    test('BiometriaIos.disponible()/confirmar() devuelven el valor seguro sin canal', () async {
    ...
    test('AvisosPushIos.token() es null sin canal, y toques no emite nada', () async {
    ...
    test('ProteccionPantallaIos.activar()/desactivar() son no-ops sin canal', () async {
