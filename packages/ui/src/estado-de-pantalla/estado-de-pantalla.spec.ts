  ...
  return (req: import('@angular/common/http').HttpRequest<unknown>, next: import('@angular/common/http').HttpHandlerFn) =>
    next(req).pipe(
      catchError((error: unknown) => {
        const e = error as { status?: number; error?: { mensaje?: string } }
        return throwError(() => ({
          mensaje: e.error?.mensaje ?? (e.status === 404 ? 'No encontramos lo que buscabas.' : e.status === 0 ? 'Sin conexión. Te mostramos lo último que vimos.' : 'Algo salió mal.'),
          estado: e.status,
...
function crearAnfitrion() {
  @Component({
    ...
    template: `
      ...
      >
        <p class="contenido">valor: {{ recurso.value()?.valor }}</p>
      </ap-estado-de-pantalla>
    `,
  })
  class Anfitrion {
...
describe('EstadoDePantalla · ramas contra ResourceStatus real (no una unión inventada)', () => {
  ...
  it('error: el mensaje NO se reescribe (viene tal cual del interceptor de cada app)', async () => {
    ...
    await fixture.whenStable()
    expect(fixture.nativeElement.querySelector('[role="alert"]')?.textContent).toContain('Sin conexión. Te mostramos lo último que vimos.')
