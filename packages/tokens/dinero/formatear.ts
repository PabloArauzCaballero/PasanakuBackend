 ...
 * del átomo `Monto` que lo usa (`planes/10` §6, regla `aportaya/sin-formato-de-dinero`). Vive en `packages/tokens` porque es la **referencia**: de acá salen los vectores de `vectores/monto.json` que el `Monto` de Angular y el de Flutter tienen que reproducir byte a byte.
 *
