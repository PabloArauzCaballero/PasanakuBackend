...
def _ddl_infra(esquema, tabla):
    ...
    if tabla == "evento_dominio":
        return [
            ...
            ");",
            f"-- Indice parcial de despacho: el relevo solo mira lo PENDIENTE listo para",
            f"-- reintentar (proximo_intento_en nulo o ya paso).",
            f"CREATE INDEX IF NOT EXISTS {ident('ix', esquema, 'evtdom', 'despacho')}",
            f"  ON {q} (ocurrido_en) WHERE estado = 'PENDIENTE';",
            f"-- Recuperacion de un TOMADO huerfano (el relevo que lo tomo murio antes",
            f"-- de marcar PUBLICADO/backoff): el siguiente relevo lo vuelve a tomar.",
            f"CREATE INDEX IF NOT EXISTS {ident('ix', esquema, 'evtdom', 'tomado')}",
            f"  ON {q} (tomado_en) WHERE estado = 'TOMADO';",
